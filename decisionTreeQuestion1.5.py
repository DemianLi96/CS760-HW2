...
def get_information_gain_ratio(dataset, idx, threshold=None):
    ...
    return information_gain / (split_information+1e-5), conditional_entropy,split_information
...
def find_best_split(dataset):
    ...
    best_split_information = 0

    for i in range(dimension_num):
        thresholds = dataset[:, i].tolist()
        for threshold in thresholds:
            gain_ratio, conditional_entropy,split_information = get_information_gain_ratio(dataset, i, threshold)
            ...
            if split_information > best_split_information:
                best_split_information = split_information
    return best_idx, best_threshold, best_gain_ratio, best_conditional_entropy,best_split_information
...
def make_subtree(dataset):
    ...
    if len(counter) == 1:
        # print("len(counter) == 1")
        return "y = "+str(dataset[0, -1])

    best_idx, best_threshold, best_gain_ratio, best_conditional_entropy,best_split_information = find_best_split(dataset)

    if best_gain_ratio <= 1e-6:
        # print("best_gain_ratio <= 1e-6")
        return "y = "+str(counter.most_common(1)[0][0])
    if best_conditional_entropy == 0:
        # print("best_conditional_entropy == 0")
        return "y = "+str(counter.most_common(1)[0][0])
    if best_split_information == 0:
        return "y = "+str(counter.most_common(1)[0][0])
    for_idx = "For x"+str(best_idx+1)
    sub_tree = {for_idx: {'if threshold is greater or equal to': best_threshold}}
    ...
    # print("")
    sub_tree[for_idx][True] = make_subtree(split_data[0])
    sub_tree[for_idx][False] = make_subtree(split_data[1])
    return sub_tree
...
def predict_label(subtree, data):
    ...
    result_label = None
    threshold = subtree['if threshold is greater or equal to']
